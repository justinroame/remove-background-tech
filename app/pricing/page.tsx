...
export default function PricingPage() {
  ...
  // ---- STRIPE HANDLER ----
  async function createCheckoutSession(amount: string, mode: "payment" | "subscription") {
    try {
      const res = await fetch("/api/create-checkout-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ priceId: amount, mode }),
      })
        ...
        <div className="mx-auto grid max-w-5xl grid-cols-1 gap-8 md:grid-cols-2">
          ...
          <Card className="flex flex-col rounded-2xl bg-white p-8 shadow-lg">
            ...
            <Button
              ...
              onClick={() => createCheckoutSession(payAsYouGoOption, "payment")}
            >
              Buy now
            </Button>
          ...
          <Card className="relative flex flex-col rounded-2xl border-2 border-yellow-400 bg-white p-8 shadow-xl">
            ...
            <Button
              className="mb-6 rounded-full bg-blue-600 py-6 text-base font-medium text-white hover:bg-blue-700"
              onClick={() => createCheckoutSession(proOption, "subscription")}
            >
              Subscribe
            </Button>
